...
try:
    ...
    sys.exit("scapy not found; install scapy.")

# minimal GTP-U header builder

def build_gtpu_header(teid:int,payload_len:int,has_optional:bool=False)->bytes:
"""
...
def craft_gtpu_packet(
        ...
        teid:int,
        payload:bytes=b"\x00"*64,)->bytes:
    """ 
    Return a complete Ethernet frame carrying a GTP-U G-PDU
    Frame layout:
    Ethernet | outer IP | UDP (sport=2152,dport=2152) | GTP-U hdr | inner IP |     payload
    """
    ...
    #build udp payload manually so scapy doesn't try to layer gtp
...
def main():
    ...
    ap.add_argument("--count",      default=10,type=float,  help="Packets to send")
    ...
    ap.add_argument("--payload-len",default=64,             help="Inner payload bytes")
    args=ap.parse_args()
    ...
    interval=1.0 / args.pp if args.pps>0 else 0
